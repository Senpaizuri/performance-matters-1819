    ...
    obj = {
        get: (url)=>{
            return new Promise(resolve =>{
                fetch(url,{header:{
                    "X-RapidAPI-Key":"a5afea7a6cmsh1c3772836574618p126520jsn977cb0af9079"
                }})
                .then(res => res.json())
        ...
        lyrics: async(data)=>{
            ...
            arr = songs.map(async(song)=>{
                let
                    ...
                    lyric = await obj.get(dataUrl,header)
    
                    if(JSON.stringify(lyric).length <= 2 || lyric.message){
